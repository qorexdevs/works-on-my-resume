...
test('repeated-word finding fires for a doubled word in a bullet', async ({ page }) => {
  const md = [
    ...
    '### Senior Engineer at Acme',
    '',
    '- Shipped the the billing rewrite two quarters early',
    '',
  ].join('\n');
  ...
  const panel = healthPanel(page);
  const finding = panel.locator('.health__list [data-rule="repeated-word"]');
  // "the the" on line 11 is a doubled word.
  await expect(finding).toHaveCount(1);
  await expect(finding.getByRole('button', { name: /jump to line 11/i })).toBeVisible();
});

test('contraction finding fires for a contraction in a bullet', async ({ page }) => {
  const md = [
    '---',
    'name: Test User',
    'role: Engineer',
    ...
    '### Senior Engineer at Acme',
    '',
    "- Automated the pipeline so builds didn't fail overnight",
    '',
  ].join('\n');
  ...
  const panel = healthPanel(page);
  const finding = panel.locator('.health__list [data-rule="contraction"]');
  // "didn't" on line 11 should read as "did not".
  await expect(finding).toHaveCount(1);
  await expect(finding.getByRole('button', { name: /jump to line 11/i })).toBeVisible();
});
...
test('verb-tense finding fires when one role mixes past and present openers', async ({
  ...
  const md = [
    ...
    '- Shipped a fraud detector that cut chargebacks',
    '- Lead the platform team day to day',
    '',
  ].join('\n');
  ...
  const panel = healthPanel(page);
  const finding = panel.locator('.health__list [data-rule="verb-tense"]');
  // Two past openers (Built, Shipped) set the role's tense; only the present
  // "Lead" on line 13 breaks it.
  await expect(finding).toHaveCount(1);
  await expect(finding.getByRole('button', { name: /jump to line 13/i })).toBeVisible();
});
